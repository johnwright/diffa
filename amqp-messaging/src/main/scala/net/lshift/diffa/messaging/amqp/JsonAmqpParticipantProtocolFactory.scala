...
class JsonAmqpParticipantProtocolFactory(connectorHolder: ConnectorHolder,
  ...
  def createUpstreamParticipant(address: String, protocol: String) =
    new UpstreamParticipantAmqpClient(connectorHolder.connector, AmqpQueueUrl.parse(address).queue, timeoutMillis)

  def createDownstreamParticipant(address: String, protocol: String) =
    new DownstreamParticipantAmqpClient(connectorHolder.connector, AmqpQueueUrl.parse(address).queue, timeoutMillis)
}