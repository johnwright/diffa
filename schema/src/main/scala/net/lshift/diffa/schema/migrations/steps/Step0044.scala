...
object Step0044 extends VerifiedMigrationStep {

  def versionId = 44
  def name = "Allow ignore escalations"

  def createMigration(config: Configuration) = {
    val migration = new MigrationBuilder(config)

    migration.alterTable("escalations").
      setColumnNullable("action", Types.VARCHAR, 50, true)
  ...
  def applyVerification(config: Configuration) = {
    ...
    migration.insert("escalations").values(Map(
      ...
      "pair_key"    -> pair,
      "name"        -> escalation,
        // No action defined, since ignore doesn't require one
      "action_type" -> "ignore",
      "event"       -> "mismatch",
      "origin"      -> "scan",
      "delay"       -> randomInt()
