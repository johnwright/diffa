...
class JSONEncodingUtilsTest {
  ...
  def queryConstraintRoundTrip = {
    ...
    val serialized = JSONEncodingUtils.serializeConstraints(Seq(constraint1, constraint2))
    val deserialized = JSONEncodingUtils.deserializeConstraints(serialized)
  ...
  @Test
  def emptyList = {
    val serialized = JSONEncodingUtils.serializeConstraints(Seq())
    val deserialized = JSONEncodingUtils.deserializeConstraints(serialized)
    assertNotNull(deserialized)
