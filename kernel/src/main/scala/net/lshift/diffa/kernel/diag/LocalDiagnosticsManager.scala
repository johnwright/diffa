...
import net.lshift.diffa.kernel.config.{Domain, DiffaPair, DiffaPairRef, DomainConfigStore}
...
class LocalDiagnosticsManager(domainConfigStore:DomainConfigStore, explainRootDir:String)
  ...
  private class PairDiagnostics(pair:DiffaPairRef) {
    ...
    private val maxExplainFiles = math.min(maxExplainFilesPerPair, pairDef.maxExplainFiles)
    private val maxEvents = math.min(maxEventsPerPair, pairDef.eventsToLog)
