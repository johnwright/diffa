 ...
 *
 * The cache operates using a pair of underlying ehcache instances - a sequence cache and an aggregate cache. The
...
class DifferenceAggregationCache(diffStore:DomainDifferenceStore, cacheManager:CacheManager) {
  val sequenceCache = new CacheWrapper[SequenceCacheKey, SequenceCacheValue]("sequencecache", cacheManager)
