...
class DefaultSessionManagerTest {
 ...
 def shouldAlwaysInformMatchEvents {
  ...
  def shouldTrackStateOfPairsForExplicitScopeSession {
    ...
    val sessionId = manager.start(SessionScope.forPairs(domainName, "pair1"), listener1)

    // Initial state of all pairs should be "unknown"
    assertEquals(Map("pair1" -> PairScanState.UNKNOWN), manager.retrievePairScanStates(sessionId))
    ...
    assertEquals(Map("pair1" -> PairScanState.SCANNING), manager.retrievePairScanStates(sessionId))

    // Notify that the pair is now in Up To Date state
    manager.pairScanStateChanged(pair1, PairScanState.UP_TO_DATE)
    assertEquals(Map("pair1" -> PairScanState.UP_TO_DATE), manager.retrievePairScanStates(sessionId))
    ...
    assertEquals(Map("pair1" -> PairScanState.FAILED), manager.retrievePairScanStates(sessionId))
  }
