...
abstract class AbstractDataDrivenPolicyTest {
  ...
  protected def expectUpstreamAggregateSync(pair:Pair, bucketing:Seq[CategoryFunction], constraints:Seq[QueryConstraint],
  ...
  protected def expectDownstreamAggregateSync(pair:Pair, bucketing:Seq[CategoryFunction], constraints:Seq[QueryConstraint],
                                              partResp:Seq[Bucket], storeResp:Seq[Bucket]) {
  ...
  protected def expectUpstreamEntitySync(pair:Pair, constraints:Seq[QueryConstraint], partResp:Seq[Vsn], storeResp:Seq[Vsn]) {
    expect(usMock.scan(constraints, Seq())).andReturn(participantEntityResponse(partResp))
  ...
  protected def expectDownstreamEntitySync(pair:Pair, constraints:Seq[QueryConstraint], partResp:Seq[Vsn], storeResp:Seq[Vsn]) {
    expect(dsMock.scan(constraints, Seq())).andReturn(participantEntityResponse(partResp))
    val correlations = storeResp.map(v=> {
...
object AbstractDataDrivenPolicyTest {
  ...
  @DataPoint def setOnlyScenario = Scenario(
    Pair(key = "ab",
      ...
      AggregateTx(Seq(byName("someString")), Seq(SetQueryConstraint("someString",Set("A"))),
      ...
      AggregateTx(Seq(byName("someString")), Seq(SetQueryConstraint("someString",Set("B"))),
      ...
      AggregateTx(Seq(byName("someString")), Seq(SetQueryConstraint("someString",Set("C"))),
  ...
  @DataPoint def dateTimesOnlyScenario = Scenario(
    ...
    AggregateTx(Seq(yearly("bizDateTime")), Seq(unboundedDateTime("bizDateTime")),
      Bucket("2010", Map("bizDateTime" -> "2010"),
        AggregateTx(Seq(monthly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2010, END_2010)),
          Bucket("2010-07", Map("bizDateTime" -> "2010-07"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", JUL_2010, END_JUL_2010)),
          ...
          Bucket("2010-08", Map("bizDateTime" -> "2010-08"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", AUG_2010, END_AUG_2010)),
      ...
      Bucket("2011", Map("bizDateTime" -> "2011"),
        AggregateTx(Seq(monthly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2011, END_2011)),
          Bucket("2011-01", Map("bizDateTime" -> "2011-01"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", JAN_2011, END_JAN_2011)),
  ...
  @DataPoint def datesOnlyScenario = Scenario(
    ...
    AggregateTx(Seq(yearly("bizDate")), Seq(unboundedDate("bizDate")),
      Bucket("1995", Map("bizDate" -> "1995"),
        AggregateTx(Seq(monthly("bizDate")), Seq(dateRange("bizDate", START_1995, END_1995)),
          Bucket("1995-04", Map("bizDate" -> "1995-04"),
            AggregateTx(Seq(daily("bizDate")), Seq(dateRange("bizDate", APR_1_1995, APR_30_1995)),
          ...
          Bucket("1995-05", Map("bizDate" -> "1995-05"),
            AggregateTx(Seq(daily("bizDate")), Seq(dateRange("bizDate", MAY_1_1995, MAY_31_1995)),
      ...
      Bucket("1996", Map("bizDate" -> "1996"),
        AggregateTx(Seq(monthly("bizDate")), Seq(dateRange("bizDate", START_1996, END_1996)),
          Bucket("1996-03", Map("bizDate" -> "1996-03"),
            AggregateTx(Seq(daily("bizDate")), Seq(dateRange("bizDate", MAR_1_1996, MAR_31_1996)),
  ...
  @DataPoint def yy_MM_dddd_dateTimesOnlyScenario = Scenario(
    ...
    AggregateTx(Seq(yearly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2023_FULL, END_2023_FULL)),
      Bucket("2023", Map("bizDateTime" -> "2023"),
        AggregateTx(Seq(monthly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2023_FULL, END_2023_FULL)),
          Bucket("2023-10", Map("bizDateTime" -> "2023-10"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", OCT_1_2023, OCT_31_2023)),
  ...
  @DataPoint def integersOnlyScenario = Scenario(
    ...
    AggregateTx(Seq(thousands("someInt")), Seq(unbounded("someInt")),
  ...
  @DataPoint def stringsOnlyScenario = Scenario(
    ...
    AggregateTx(Seq(oneCharString("someString")), Seq(unbounded("someString")),
      Bucket("A", Map("someString" -> "A"),
  ...
  @DataPoint def integersAndDateTimesScenario = Scenario(
    ...
    AggregateTx(Seq(yearly("bizDateTime"), thousands("someInt")), Seq(unboundedDateTime("bizDateTime"), unbounded("someInt")),
      Bucket("2010_1000", Map("bizDateTime" -> "2010", "someInt" -> "1000"),
        AggregateTx(Seq(monthly("bizDateTime"), hundreds("someInt")), Seq(dateTimeRange("bizDateTime", START_2010, END_2010), intRange("someInt", 1000, 1999)),
          Bucket("2010-07_1200", Map("bizDateTime" -> "2010-07", "someInt" -> "1200"),
            AggregateTx(Seq(daily("bizDateTime"), tens("someInt")), Seq(dateTimeRange("bizDateTime", JUL_2010, END_JUL_2010), intRange("someInt", 1200, 1299)),
          ...
          Bucket("2010-08_1300", Map("bizDateTime" -> "2010-08", "someInt" -> "1300"),
            AggregateTx(Seq(daily("bizDateTime"), tens("someInt")), Seq(dateTimeRange("bizDateTime", AUG_2010, END_AUG_2010), intRange("someInt", 1300, 1399)),
      ...
      Bucket("2011_2000", Map("bizDateTime" -> "2011", "someInt" -> "2000"),
        AggregateTx(Seq(monthly("bizDateTime"), hundreds("someInt")), Seq(dateTimeRange("bizDateTime", START_2011, END_2011), intRange("someInt", 2000, 2999)),
          Bucket("2011-01_2300", Map("bizDateTime" -> "2011-01", "someInt" -> "2300"),
            AggregateTx(Seq(daily("bizDateTime"), tens("someInt")), Seq(dateTimeRange("bizDateTime", JAN_2011, END_JAN_2011), intRange("someInt", 2300, 2399)),
  ...
  @DataPoint def setAndDateTimesScenario = Scenario(
    ...
    AggregateTx(Seq(yearly("bizDateTime"), byName("someString")), Seq(unboundedDateTime("bizDateTime"), SetQueryConstraint("someString",Set("A"))),
      Bucket("2010_A", Map("bizDateTime" -> "2010", "someString" -> "A"),
        AggregateTx(Seq(monthly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2010, END_2010), SetQueryConstraint("someString",Set("A"))),
          Bucket("2010-07_A", Map("bizDateTime" -> "2010-07"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", JUL_2010, END_JUL_2010), SetQueryConstraint("someString",Set("A"))),
    ...
    AggregateTx(Seq(yearly("bizDateTime"), byName("someString")), Seq(unboundedDateTime("bizDateTime"), SetQueryConstraint("someString",Set("B"))),
      Bucket("2011_B", Map("bizDateTime" -> "2011", "someString" -> "B"),
        AggregateTx(Seq(monthly("bizDateTime")), Seq(dateTimeRange("bizDateTime", START_2011, END_2011), SetQueryConstraint("someString",Set("B"))),
          Bucket("2011-01_B", Map("bizDateTime" -> "2011-01"),
            AggregateTx(Seq(daily("bizDateTime")), Seq(dateTimeRange("bizDateTime", JAN_2011, END_JAN_2011), SetQueryConstraint("someString",Set("B"))),
  ...
  def yearly(attrName:String) = YearlyCategoryFunction(attrName)
  def monthly(attrName:String) = MonthlyCategoryFunction(attrName)
  def daily(attrName:String) = DailyCategoryFunction(attrName)
   ...
   */
  case class AggregateTx(bucketing:Seq[CategoryFunction], constraints:Seq[QueryConstraint], respBuckets:Bucket*) extends Tx {
