...
class HibernateConfigStoreTest {
  ...
  val UPSTREAM_EP = new Endpoint("TEST_UPSTREAM", "TEST_UPSTREAM_URL", true)
  val DOWNSTREAM_EP = new Endpoint("TEST_DOWNSTREAM", "TEST_DOWNSTREAM_URL", true)
  ...
  def setUp: Unit = {
    ...
    Seq("Pair", "PairGroup", "Endpoint") foreach (tableName => {
      s.createQuery("DELETE FROM " + tableName).executeUpdate
    })
    s.flush
    s.close
  }
  ...
  def testUpdateEndpoint: Unit = {
    ...
    // Change its name
    configStore.createOrUpdateEndpoint(new Endpoint(UPSTREAM_RENAMED, UPSTREAM_EP.url, true))
  ...
  def testRedeclareEndpointSucceeds = {
    configStore.createOrUpdateEndpoint(UPSTREAM_EP)
    configStore.createOrUpdateEndpoint(new Endpoint(UPSTREAM_EP.name, "DIFFERENT_URL", false))
