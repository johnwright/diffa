...
class CorrelatedVersionPolicyTest extends AbstractPolicyTest {
  ...
  protected def shouldUpdateDownstreamVersionsWhenStoreIsOutOfDateWithDownstreamParticipant(
    ...
    expect(session.storeDownstreamVersion(VersionID(abPair, "id3"), testData.downstreamAttributes(1), JUL_8_2010_1, "vsn3", downstreamVersionFor("vsn3"))).
    ...
    expect(session.clearDownstreamVersion(VersionID(abPair, "id4"))).
    ...
    expect(session.storeDownstreamVersion(VersionID(abPair, "id5"), testData.downstreamAttributes(1), JUL_8_2010_1, "vsn5a", downstreamVersionFor("vsn5a"))).
    ...
    policy.difference(abPair, session, usMock, dsMock, nullListener)
    verifyAll
  }
